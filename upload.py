    ...
    if file_path:
        with open(file_path, 'rb') as pdf_file:
            ...
            for sentence in sentences:
                ...
                if len(current_chunk) + len(sentence) + 1 < 1200:  # +1 for the space
                    current_chunk += (sentence + " ").lower()
                else:
                    # When the chunk exceeds 1200 characters, store it and start a new one
                    chunks.append(current_chunk)
                    current_chunk = (sentence + " ").lower()
            if current_chunk:  # Don't forget the last chunk!
                chunks.append(current_chunk)
            with open(".cache/vault.txt", "a", encoding="utf-8") as vault_file:
                for chunk in chunks:
